...
/*
 * SSDT устройства Hyper-V, отключающий неподдерживаемые устройства для macOS
 * на Windows Server2019 / Windows10 и новее.
 *
 * Windows10 и новее условно определяют различные объекты, которые
 * не поддерживаются в старых версиях macOS. В старых версиях все объекты
 * включаются независимо от состояния, что вызывает медленную загрузку и
 * аварии, связанные с AppleACPIPlatform.
 *
 * Windows10 и новее также используют объекты ACPI0007 для CPU, которые
 * macOS не может обнаружить. Этот SSDT переопределяет первые64 как
 * объекты Processor. Это может вызвать проблемы в других ОС.
 *
 * Этот SSDT определяет методы _STA для всех затронутых объектов и полностью
 * отключает их, либо включает только если соответствующий регистр установлен
 * (включён в Hyper-V).
 *
 * На Windows Server2012 R2 / Windows8.1 и старее этот SSDT не нужен и может
 * быть отключён вместе с соответствующими патчами.
 *
 * Требует следующих ACPI патчей:
...
DefinitionBlock ("", "SSDT",2, "ACDT", "HVDEV",0x00000000)
{
 ...
 //
 // Duplicated from Windows8.1 Hyper-V Generation2 DSDT.
 // Defines old-style Processor objects that macOS can use.
 //
 // PSTA will exist on8.1 and older, and should block this SSDT from
 ...
 //
 // Older versions of macOS before10.6 do not support conditional definitions
