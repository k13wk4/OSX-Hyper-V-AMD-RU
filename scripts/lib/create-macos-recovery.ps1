...
# Создаём outdir, если не существует
New-Item -ItemType Directory "$outdir" -Force | Out-Null

# Синхронизовано со https://github.com/acidanthera/OpenCorePkg/blob/master/Utilities/macrecovery/recovery_urls.txt
switch ($version)
{
 # Lion (10.7)
 ...
 # Latest (по умолчанию Sequoia15.6)
 Default { $b="Mac-CFF7D910A743CAAF"; $m="00000000000000000"; }
}

# Запуск macrecovery.py
...
# Проверяем, что outdir создан
if (-not (Test-Path -Path $outdir)) {
 Write-Error "Не удалось создать директорию восстановления по пути $outdir. Проверьте скрипт и попробуйте снова."
 exit1
}
